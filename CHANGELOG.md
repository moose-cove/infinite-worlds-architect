...
- **`firedThisTurn` semantics confirmed, and two new `triggerOnPawScript` warnings (v0.23.0).** Probe C (2026-08-29, round trip + played session) closed the two oldest open probe questions. **`firedThisTurn` selects the match window** on both `triggerPrereqs` and `triggerBlockers`: `false` means the listed trigger fired at any point in the past, `true` narrows it to the current turn — a same-turn interlock. Confirmed against a one-shot anchor across three turns, all four cells behaving as the reading predicts, with the rival "platform-managed runtime state" reading ruled out. The plugin still only type-checks the field, which is now correct rather than cautious since both values are legitimate; the docs explain when to reach for each, including the pass-order trap (list order is evaluation order, so a `firedThisTurn: true` consumer must sit after its anchor). **Two new warnings**, both for shapes IW keeps on import and then fails on every turn: `<<name>>` interpolation used as a bare operand (IW substitutes the item's *value* textually before parsing, so `<<flavor>>` holding `Lemon` becomes the expression `Lemon = "Lemon"` and errors on the unknown name — interpolation *inside* a quoted literal is legitimate and is not flagged), and a bare `$handle` with no comparison (it works out to a value, so unless that value is itself true/false the trigger never fires). The interpolation check covers both expression condition types, since the substitution is pre-parse — though only `triggerOnPawScript` was probed, so the `triggerOnRandomChance` case is flagged as a caution. The bare-handle check is `triggerOnPawScript`-only: a bare `$handle` is the correct form for a chance formula. Also confirmed: an auto-created per-character entry is **seeded from the tracked item's `initialValue`**, which upgrades `TRACKED_ITEMS.md`'s "drop the entry and set the value on the item" from a hedged suggestion to a confirmed fix. (`src/iw_architect/validator.py`, `references/world_v2.4.schema.json`, `references/WORLD_JSON_SCHEMA_v2.4.md`, `references/fields/TRIGGER_EVENTS.md`, `references/fields/TRACKED_ITEMS.md`, `references/mechanics/PAWSCRIPT.md`, `references/mechanics/PLATFORM_BEHAVIOR_NOTES.md`, `agents/world-architect.md`, `probes/`, `tests/test_validator.py`)

- **Probe E — the entry-vs-item scope confound resolved, and a new doomed-entry warning (v0.22.0).** Two live import/export round trips plus a two-turn played session (2026-08-28) closed three documented unknowns. **`initialValueBasedOnPC`:** the delete is keyed on the incoming *entry's* own scope value — item `"character"` + entry `"player"` was deleted, while item `"player"` + entry `"character"` survived one import but exported with its scope **rewritten to the item's**, so the next import deleted it. Entry scope is a projection of the item's, not independent state, and a `"character"`-scoped item that arrives with no entry gets one auto-created. The existing entry-level error stands (now unhedged) and a **new warning** flags an entry backed by a `"player"`-scoped item — a state that survives one import and vanishes on the next. This is also the first observed case of an IW export that IW's own import then mutates, so `probes/probe-e-imported.json` is the one `-imported` evidence file that correctly fails validation. **`recommendedAIModel`:** `"notarealmodel"` survived both round trips verbatim — there is no import-time enum check, closing the `DESIGN_BRIEF_v2.md` §9 enum question as unanswerable by construction (survival, not semantics: retired-name stripping and runtime honoring stay untested). **Q10:** an absent `triggerConditions` key is normalized to `[]` at import and is runtime-dead in play (the control fired 3/3 turns, the absent-key trigger never), so the conditionless-trigger warning covers both shapes with no remaining hedge. Harness: `iwdrive.py`'s `import_json` now polls for and dismisses the delayed "World imported from raw JSON." alert before clicking Save — it was intercepting the click and timing the flow out — and gains a `recover` command. (`src/iw_architect/validator.py`, `references/world_v2.4.schema.json`, `references/WORLD_JSON_SCHEMA_v2.4.md`, `references/fields/TRACKED_ITEMS.md`, `references/fields/TRIGGER_EVENTS.md`, `references/mechanics/PLATFORM_BEHAVIOR_NOTES.md`, `references/mechanics/AI_RUNTIME_MECHANICS.md`, `probes/`, `tests/test_validator.py`, `dev-docs/DESIGN_BRIEF_v2.md`)

- **A conditionless trigger never fires; a tracked item without `description` bricks the editor (v0.21.0).** In-platform play on 2026-08-22, then Probe D (imported, re-exported with all 21 conditions byte-identical, and played through the new Playwright/CDP harness). **The headline:** a trigger with `triggerConditions: []` never fires, on any turn, even with `canTriggerMoreThanOnce: true` — so the legacy bare-array gate deletion leaves a **dead** trigger, not an ungated one, which is the strictly worse of the two readings the docs had been hedging between. New Tier-2 warning `_check_conditionless_triggers` (non-SoG trigger with empty or absent conditions); the `triggerOnStartOfGame` + no-conditions case fires on turn 0, confirming the validator's SoG exemption rather than merely permitting it. New Tier-2 **error** `_check_tracked_item_description_present`: a tracked item with no string `description` imports and plays, but the IW editor then silently refuses to open the world — and the editor is the only route back to the raw-JSON box, so the world is stuck until the file is fixed (bisected during the Probe D build). PawScript runtime confirmations: `and`/`or`/`not` work in conditions and `or` short-circuits left-to-right; `==` is a syntax error; a non-boolean condition never fires; every numeric `triggerOnRandomChance` form fires (bare `$handle`, additive, `choose()`, `if(cond,a,b)`, both `turn_number` spellings) while a comparison used as a number errors because it yields text; same-turn script writes are visible to later triggers in the same pass; triggers run in list order; an errored one-shot still consumes the trigger *and* still satisfies `triggerPrereqs`; multi-level and `.item($k)` path creation both work. The `probes/harness/` Playwright/CDP scripts land here, with the World Debug, tracked-items and Expression Sandbox transcripts as committed evidence. (`src/iw_architect/validator.py`, `references/world_v2.4.schema.json`, `references/WORLD_JSON_SCHEMA_v2.4.md`, `references/fields/TRIGGER_EVENTS.md`, `references/fields/YAML_TRACKED_ITEMS.md`, `references/mechanics/PAWSCRIPT.md`, `references/mechanics/PLATFORM_BEHAVIOR_NOTES.md`, `agents/world-architect.md`, `probes/`, `tests/test_validator.py`, `tests/test_analysis.py`)

- **`triggerOnRandomChance` formulas can reference tracked items (v0.20.0).** The canonical fixture (now world version 1.1, exported 2026-08) changed its 'Random Newcomer' chance formula from `"15+round(turn_number%random)"` to `"$number_of_non_human_friends+round(turn_number%random)"` — a tracked item read in the PawScript `$variableName` form, so the odds scale with state. The schema artifact's `x-iw-condition-types.triggerOnRandomChance` entry, `WORLD_JSON_SCHEMA_v2.4.md`, `TRIGGER_EVENTS.md`, `PAWSCRIPT.md` and the `world-architect` agent now document the form (and its mixed dialect: `turn_number` / `random` stay bare while the tracked item carries the `$`). `validate_world`'s expression-condition check, previously `triggerOnPawScript`-only, now covers `triggerOnRandomChance` too: it warns on a non-string/blank formula and on a `$name` that is not a tracked item's `variableName` or a native, with the bare tokens left unchecked. Whether the `$` form actually *evaluates* at runtime (and whether `$game.turn_number` works in that field) is open — `probes/README.md` P15. (`references/world_v2.4.schema.json`, `references/WORLD_JSON_SCHEMA_v2.4.md`, `references/fields/TRIGGER_EVENTS.md`, `references/mechanics/PAWSCRIPT.md`, `agents/world-architect.md`, `src/iw_architect/validator.py`, `tests/test_validator.py`, `probes/README.md`)
...
- **A blank or absent `triggerOnPawScript` `data` is now an error, not a warning (v0.23.0).** Probe C imported both shapes and neither is survivable: an absent `data` key leaves no condition in the export and no World Debug entry at all, so the gate does nothing, while a blank string is **dropped from the exported JSON but still evaluated every turn**, where it errors with `Empty expression - nothing to evaluate`. Either way the gate does no gating, which puts these alongside the existing `textComparison` errors rather than in the warnings. (What the round trip cannot settle: because both cells export as `triggerConditions: []`, it cannot distinguish deletion-at-import from kept-but-never-logged for the absent-key case. The authoring consequence is identical, which is what the severity rests on.) Scoped deliberately: a present-but-non-string `data`, and the `triggerOnRandomChance` equivalent, were not probed and stay warnings. The blank-`data` case is also the first construct known to **differ between an IW export and the live world** — a re-export diff can under-report as well as over-report, so "it's not in the export" is not proof the platform stopped running it (`PLATFORM_BEHAVIOR_NOTES.md`). Probe C also bounded the doc's standing generalisation about condition deletion: it is **structural, not semantic** — IW deletes a condition whose `data` is the wrong shape or absent, but keeps a syntactically-present string it cannot evaluate and lets it fail at runtime (an undeclared `$name`, a non-boolean expression and a `<<…>>` interpolation all survived import verbatim). (`src/iw_architect/validator.py`, `references/world_v2.4.schema.json`, `references/mechanics/PLATFORM_BEHAVIOR_NOTES.md`, `references/mechanics/PAWSCRIPT.md`, `tests/test_validator.py`)
...
- **`test_make_draft_world_copies_bumps_and_fronts_version` now derives the expected bump through `_bump_version_component`** instead of its own `int(minor) + 1` arithmetic, which ignored the two-decimal-place widening rule and demanded `1.1 → 1.2` once the fixture refreshed to version `1.1`. That refresh is also the first platform-side evidence for the v0.19.1 rule: IW exported 1.10 as `"1.1"`, so the trailing zero really is dropped by the platform, not by hand — noted in the helper's docstring. (`tests/test_round_trip.py`, `src/iw_architect/tools/helpers.py`)

- **`make_draft_world` bumped `1.3` to `1.4` instead of `1.31` (v0.19.1).** IW world versions are two-decimal-place numbers, so a single-digit minor is a trailing-zero version with the zero dropped (`1.3` is 1.30) and the old "+1 on the last component" logic skipped nine versions. `_bump_version_component` now widens a sub-two-digit minor to two places before incrementing — `1.3 → 1.31`, `1.0 → 1.01` — while `1.09 → 1.10`, `1.99 → 1.100`, and bare `5 → 6` are unchanged. The same helper drives the `_v<ver>` filename token, so `world_v1.3.json` now drafts to `world_v1.31_draft.json`. (`src/iw_architect/tools/helpers.py`, `tests/test_round_trip.py`)

- **`extract_story_data`'s character index no longer counts tracked-item tables as mentions.** `index_characters` scanned every raw line inside a turn's `line_range`, including the `Tracked Items` / `Hidden Tracked Items` sections. Any tracked-item label or value that embeds a character's name (`Sage's Hound Status:`, a per-turn traits row like `Kelsey Braddock: Wide-eyed, follower`) therefore registered as a "mention" on every turn of the export — on a real 147-turn export, hundreds of phantom mentions per character, drowning the narrative mentions the index exists to surface. The indexer now tracks the current section header while it walks the lines (a header line followed by a `----` rule, the same shape `sections.py` recognises) and skips those two sections; header and rule lines are never indexed; a `-- Turn N --` marker resets the state so a turn without headers is still scanned in full. Action / Outcome / Secret Information are unchanged. (`src/iw_architect/story/characters.py`, `tests/story/test_characters.py`)

...
Returns a :class:`~iw_architect.story.models.CombineResult` with ``header``,
``turns`` (a list of :class:`~iw_architect.story.models.RawTurn`,
deduplicated and sorted ascending by number), ``combined_text``, and
``warnings``.
"""
...
from iw_architect.story.models import CombineResult, RawTurn
...
def _split_turns(text: str, source: str, mtime: float) -> tuple[str, list[RawTurn]]:
    """Return (header_text, [RawTurn]) for one file's normalised text."""
    ...
    for i, m in enumerate(matches):
        ...
        turns.append(
            RawTurn(number=number, content=content, source=os.path.abspath(source), mtime=mtime)
        )
    return header, turns


def combine(file_paths: list[str]) -> CombineResult:
    ...
    Returns
    -------
    A :class:`~iw_architect.story.models.CombineResult`.
    ...
    all_turns: dict[int, RawTurn] = {}  # number → RawTurn (newest mtime wins)
    header_text = ""

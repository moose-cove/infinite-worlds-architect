...
description: >
  ...
  user: "I updated WORLD_JSON_SCHEMA_v2.4.md to explain how multiple conditions on a trigger are combined — I said they AND together."
  ...
  user: "I added a new optional `voiceDescription` field to characters in world_v2.4.schema.json, updated WORLD_JSON_SCHEMA_v2.4.md, and added a cross-reference check in validator.py."
...
1. **`references/world_v2.4.schema.json`** — the canonical JSON Schema. AUTHORITATIVE for what fields exist, what values are valid, what's required, and what types things must be. Pay attention to `x-iw-category`, `x-iw-note`, `x-iw-effect-types`, `x-iw-condition-types`, and other `x-iw-*` extensions — these carry IW-specific semantics that the bare JSON Schema vocabulary doesn't express.

2. **`example-world-schema-v2.4.json`** (at the repo root) — the canonical fixture. AUTHORITATIVE for "what a well-formed world looks like." Per the project's `CLAUDE.md`: *if the schema and the fixture conflict, the fixture wins* — the validator must be fixed to accept the fixture, not the other way around.

3. **`references/WORLD_JSON_SCHEMA_v2.4.md`** — human-readable schema explanation. Useful for narrative context and intent. Defers to the JSON schema on any factual conflict. **When this doc is the artifact under review, treat it as a hypothesis to verify against tiers 1 and 2, not a source of truth.**
...
3. **Schema/doc sync** — If the doc describes a field, does the field actually exist in `world_v2.4.schema.json`? Does the doc's description match the schema's `description` and `x-iw-note`? Drift is `HIGH` severity.
...
2. **Fixture still validates** — Per the "fixture wins" rule, the canonical `example-world-schema-v2.4.json` must still validate under the modified schema. If not, that is `CRITICAL`.
...
5. **Reference doc sync** — Verify `WORLD_JSON_SCHEMA_v2.4.md` was updated to describe the new field/type.
...
3. **Reference doc third** — Pull explanatory context from `WORLD_JSON_SCHEMA_v2.4.md` if the schema and fixture alone don't answer the question.

...
- **Compound or computed state tests belong in one `triggerOnPawScript` condition, not a stack of `triggerOnTrackedItem` under a `logic` node.** Its `data` is a single PawScript boolean expression in the `$variableName` form (no `<<…>>`), e.g. `"$favorite_flavor = \"Lemon\""` — the fixture 1.09 sample. Every `$name` must be a tracked item's `variableName` or a native (`$player`/`$game`); `validate_world` warns on anything else and on a `data` that is not a non-empty string. It is deterministic (not AI-evaluated), so it is presumed not to count toward the ten-event cap — unverified, as is what the platform does with a malformed expression. Keep `triggerOnTrackedItem` for single-clause comparisons: it is the older, probe-confirmed shape. See `references/fields/TRIGGER_EVENTS.md` (Choosing condition types) and `references/mechanics/PAWSCRIPT.md` §3.
- **A `triggerOnRandomChance` formula can read a tracked item as `$variableName` — use that for "chance scales with X".** Fixture 1.1 (2026-08): `"$number_of_non_human_friends+round(turn_number%random)"`. Do not build a `triggerOnPawScript` gate plus a separate roll for this; put the variable in the formula. The dialect is mixed — keep `turn_number` and `random` bare (no `$`, no `$game.` prefix, no `random(…)`) and carry the `$` only on the tracked item, exactly as the fixture does. `validate_world` warns on a `$name` that is not a `variableName` or native and on a non-string/blank formula.

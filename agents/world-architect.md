...
- **Per-character `initialTrackedItemValues` entries belong on `"character"`-scoped items only.** IW deletes on import any entry whose own `initialValueBasedOnPC` is `"player"` (whatever the backing item says), and an entry backed by a `"player"`-scoped *item* survives one import but is silently deleted on the next round trip — the export rewrites entry scopes to the item's (Probe E, 2026-08-28). `validate_world` errors on the first and warns on the second. When authoring: item `"character"` + entry `"character"`, or no entry at all.

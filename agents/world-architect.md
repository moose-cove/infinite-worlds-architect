...
- **A `triggerOnPawScript` must carry a real comparison in `data`.** Blank or absent `data` is fatal — IW deletes the condition on import or errors on it every turn (`validate_world` errors). Three shapes survive import and then die silently in play: an undeclared `$name`, a bare `$handle` (a value, never true/false), and `<<name>>` interpolation, which IW substitutes textually so `<<flavor>>` holding `Lemon` becomes `Lemon = "Lemon"` and errors. Write `$flavor = "Lemon"`. Probe C, 2026-08-29.
...
- **Compound or computed state tests belong in one `triggerOnPawScript` condition, not a stack of `triggerOnTrackedItem` under a `logic` node.** Its `data` is a single PawScript boolean expression in the `$variableName` form (no `<<…>>`), e.g. `"$favorite_flavor = \"Lemon\""` — the fixture 1.09 sample. Every `$name` must be a tracked item's `variableName` or a native (`$player`/`$game`); `validate_world` warns on anything else and on a `data` that is not a non-empty string. It is deterministic (not AI-evaluated), so it is presumed not to count toward the ten-event cap — unverified. A malformed or non-boolean expression survives import and simply never fires (2026-08-22). `and`/`or`/`not` work, `or` short-circuits left-to-right (put the branch that may not exist on the right), and equality is a single `=`. Keep `triggerOnTrackedItem` for single-clause comparisons: it is the older, probe-confirmed shape. See `references/fields/TRIGGER_EVENTS.md` (Choosing condition types) and `references/mechanics/PAWSCRIPT.md` §3.

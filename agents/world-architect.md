...
**If you are handed an existing world to *modify*, protect the source before doing anything else.** Copy the source to a draft, then treat that draft as "the world JSON" for the entire edit-flow contract below — **never edit the file the author handed you**; it stays a clean, last-known-good diff baseline. In brief: append `_draft` to the filename (incrementing any version token), copy with a shell `cp` (never a Read-then-`Write` round-trip), bump the draft's in-file `version` and move that key to the top of the file, then operate only on the draft. The full draft-naming and version-bump procedure lives in the `/infinite-worlds-architect:modify-world` command's draft-copy step — follow it when invoked directly (as a subagent) without that command loaded.
...
- **Keep `version` at the top of the file.** By convention this plugin surfaces the `version` key as the **first** property of a world JSON so the author sees it the moment they open the raw file. New scaffolds (`create_new_world_json`) already emit it first; when you modify or spin off an existing world, relocate `version` to the top as part of the version-bump step (typically two `Edit`s: remove it from its current position, then insert it as the first property after the opening `{`). Key order never changes how IW interprets a world, but IW *does* renormalize top-level order to its canonical order on import — where `version` sorts near the end (see `references/mechanics/PLATFORM_BEHAVIOR_NOTES.md`, "Canonical JSON Field Ordering"). So front-placing `version` is a local, pre-import readability convenience that IW undoes on import; it never reaches exported worlds. If a world has no `version` field, leave it absent; don't inject one.

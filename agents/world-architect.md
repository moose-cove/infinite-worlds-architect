...
**If you are handed an existing world to *modify*, protect the source before doing anything else.** Copy the source to a draft, then treat that draft as "the world JSON" for the entire edit-flow contract below — **never edit the file the author handed you**; it stays a clean, last-known-good diff baseline. In brief: append `_draft` to the filename (incrementing any version token), copy with a shell `cp` (never a Read-then-`Write` round-trip), bump the draft's in-file `version`, then operate only on the draft. The full draft-naming and version-bump procedure lives in `/infinite-worlds-architect:modify-world` **Step 2** — follow it when invoked directly (as a subagent) without that command loaded.

...
- **`possibleCharacters[*].initialTrackedItemValues[*].initialPCValue`** can be a **string OR a string array** — and it is this *shape*, not the `initialValueBasedOnPC` enum, that determines whether a choice is offered:
  ...
  - **What the experiment did *not* separate — read before relying on the scope claim.** In every cell of both probes, the entry's `initialValueBasedOnPC` and its backing tracked item's `initialValueBasedOnPC` were held **equal**. So the evidence is equally consistent with two readings: *(a)* a player-scoped **entry** is deleted, or *(b)* a player-scoped **item** drops its per-character entries. Reading (b) is arguably the more natural implementation, and the two decisive mixed cells (item `"player"` + entry `"character"`, and the reverse) have **never been imported**. `validate_world` errors on the entry regardless — that is correct in every observed case, and both readings agree the world is broken — but do not cite this as proof that the item's setting is irrelevant.
  - **Fixing it.** Set **both** the entry and its tracked item to `"character"`, or drop the entry and let the item's own setting stand. Changing only the entry is untested: under reading (b) it would not help. What *is* separately established is that `"player"` on the tracked item alone doesn't destroy the item — every such item round-tripped byte-identical, including the ones backing deleted entries.

...
- **`possibleCharacters[*].initialTrackedItemValues[*].initialPCValue`** can be a **string OR a string array** — and it is this *shape*, not the `initialValueBasedOnPC` enum, that determines whether a choice is offered:
  ...
  - **`"player"` on a per-character entry is fatal.** Confirmed 2026-08-06 by round trip: IW **deletes** any `initialTrackedItemValues` entry whose `initialValueBasedOnPC` is `"player"`, at either value shape. The 2×2 varied value shape against scope — `array + "character"` and `string + "character"` both survived; `array + "player"` and `string + "player"` were both deleted. That **clears the array form** (the pre-probe theory blamed it) and pins the fatal factor to `"player"`. Semantically coherent: `"player"` means the value is *not* per-character, so a per-character override contradicts itself.
  - **Which level is fatal — resolved 2026-08-28 (Probe E).** The two mixed cells were imported and the answer is entry-level, with a deeper model behind it. Item `"character"` + entry `"player"`: the entry is **deleted** — import keys the delete on the incoming *entry's* own scope value, whatever the item says. Item `"player"` + entry `"character"`: the entry **survives that import**, but IW exports it with its scope **rewritten to the item's** (`"player"`), so the *next* import deletes it. Entry-level `initialValueBasedOnPC` is not an independent field: IW stores and exports it as a projection of the item's, and a `"character"`-scoped item with no entry even gets one auto-created (empty value) on import. Practical upshot: per-character entries belong on `"character"`-scoped items, full stop. `validate_world` errors on a player-scoped entry and warns on any entry backed by a player-scoped item (the doomed-next-round-trip state).
  - **Fixing it.** Scope the entry **and** its tracked item `"character"`, or drop the entry and let the item's own setting stand. `"player"` on the tracked item alone doesn't destroy the item — every such item round-tripped byte-identical — it just can't meaningfully carry per-character entries.

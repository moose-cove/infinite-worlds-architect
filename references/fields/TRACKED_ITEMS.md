...
- **`possibleCharacters[*].initialTrackedItemValues[*].initialPCValue`** can be a **string OR a string array** — and it is this *shape*, not the `initialValueBasedOnPC` enum, that determines whether a choice is offered:
  ...
  - **Per-character entries require a `"character"`-scoped item — `"player"` at either level is fatal.** Set the tracked item *and* the entry to `"character"`. An entry with `initialValueBasedOnPC: "player"` is deleted on the next import, whatever the backing item says (confirmed 2026-08-06; the 2×2 cleared the value shape — arrays are fine; Probe E 2026-08-28 pinned the delete to the entry's own scope value). An entry under a `"player"`-scoped *item* survives one import, but IW exports entry scopes rewritten to the item's, so the round trip after that deletes it (Probe E). `validate_world` errors on the first and warns on the second.
  - **Why it's coherent.** Entry-level `initialValueBasedOnPC` is not independent state: IW stores and exports it as a projection of the item's, and a `"character"`-scoped item that *arrives with no entry* gets one auto-created on import, **seeded from the item's `initialValue`** (Probe C, 2026-08-29 — an item carrying `"PROBE INHERIT SEED"` produced an entry with exactly that `initialPCValue`; Probe E's `""` result was its item's empty `initialValue` showing through, not a blanking rule). A deleted entry is not re-created in the same import. `"player"` on the item alone is harmless to the item — every such item round-tripped byte-identical — it just can't carry per-character entries. So "just drop the entry and set the value on the item" is a **real fix**, not a lossy one: IW materializes the entry for you and the item's value carries into it.

...
- **`possibleCharacters[*].initialTrackedItemValues[*].initialPCValue`** can be a **string OR a string array** — and it is this *shape*, not the `initialValueBasedOnPC` enum, that determines whether a choice is offered:
  ...
  - **`"player"` on a per-character entry is fatal.** Confirmed 2026-08-06 by round trip: IW **deletes** any `initialTrackedItemValues` entry whose `initialValueBasedOnPC` is `"player"`, at either value shape. A 2×2 isolated it to that field alone — `array + "character"` and `string + "character"` both survived; `array + "player"` and `string + "player"` were both deleted. So the array form is fine; `"player"` is the problem. This is semantically coherent: `"player"` means the value is *not* per-character, so a per-character override contradicts itself.
  - **Scope matters — this is about the entry, not the item.** `"player"` is perfectly valid on the tracked item itself (`trackedItems[*].initialValueBasedOnPC`); every such item round-tripped byte-identical in the same import, including the ones backing the deleted entries. Use `"character"` on the entry, or drop the entry and let the item's own setting stand. `validate_world` errors on the entry form.

...
- **`possibleCharacters[*].initialTrackedItemValues[*].initialPCValue`** can be a **string OR a string array** — and it is this *shape*, not the `initialValueBasedOnPC` enum, that determines whether a choice is offered:
  ...
  - **Why it's coherent.** Entry-level `initialValueBasedOnPC` is not independent state: IW stores and exports it as a projection of the item's, and a `"character"`-scoped item that *arrives with no entry* gets one auto-created (`initialPCValue: ""`) on import — a deleted entry is not re-created in the same import. `"player"` on the item alone is harmless to the item — every such item round-tripped byte-identical — it just can't carry per-character entries. One consequence for "just drop the entry" as a fix: on a `"character"`-scoped item, IW materializes an empty entry in its place, and whether the item-level `initialValue` still applies then is untested (Probe E's cell had `initialValue: ""` too, so it can't distinguish) — prefer setting the value explicitly.

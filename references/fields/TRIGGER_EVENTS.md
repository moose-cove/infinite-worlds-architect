...
gated control, and both `effectShowMessage` popups appeared before the character
screen. The flag is its own gate, which is why the validator exempts SoG triggers.
...
- **Put the consumer after its anchor in `triggerEvents`.** Pass order is list order (Probe D Q6), and Probe C always used that arrangement — so the reverse is untested, and leading with the anchor is the safe default rather than a measured requirement.
...
| Scripted state test (compound, arithmetic, or YAML sub-field) | `triggerOnPawScript` — `data` is one PawScript boolean expression, e.g. `$favorite_flavor = "Lemon"` or `$gold >= 50 and $puppies.count > 2`. Evaluated deterministically each turn against live tracked-item values (`$variableName` form, not `<<…>>`). **Play-confirmed 2026-08-22** — numeric comparison against live values, dot-path reads into a `dataType: "yaml"` item, true/false gating and the one-shot default all behaved as documented. This is the deterministic gate to reach for: use it instead of stacking `triggerOnTrackedItem` under a `logic` node once the test has more than one clause or needs a computed value, and instead of synthesizing a boolean inside a `triggerOnRandomChance` formula (see the callout below). Not AI-evaluated, so presumed **not** to count toward the ten-event cap (unverified). Every `$name` must be a tracked item's `variableName` or a native — the validator warns on anything else. `and` / `or` / `not` work; `or` short-circuits left-to-right, so put a path that may not exist on the right (Probe D, 2026-08-22). **`data` must be a non-empty string — blank or absent is a validator error**, because the gate then does no gating at all (Probe C, 2026-08-29). A non-boolean or erroring expression never fires and never misfires, but it is not invisible: World Debug's PawScript panel names each failure and quotes the expression. `<<…>>` is not merely the wrong style — IW substitutes the value *before* parsing, so `<<flavor>>` holding text becomes a bare word and the gate dies. See [`PAWSCRIPT.md` §3](../mechanics/PAWSCRIPT.md#3-expressions-). |
| Probabilistic firing | `triggerOnRandomChance` — formula string (e.g., `"30"` for 30%, or `"15+round(turn_number%random)"` for dynamic). The formula can read a tracked item as `$variableName` — fixture 1.1 uses `"$number_of_non_human_friends+round(turn_number%random)"` so the chance grows with state. That is the idiom for "chance scales with X"; it does not need a `triggerOnPawScript` plus a separate roll. The fixture writes `turn_number` and `random` bare (no `$`); `$game.turn_number` resolves here too (Probe D, 2026-08-22). A bare `$handle`, the additive idiom, `choose(…)` and `if(cond, a, b)` are all play-confirmed, and a `$handle` holding 0 never fires — the value drives the roll. Treat the field as a **numeric** expression, never a boolean one — see the callout below. The validator warns on a `$name` that is not a `variableName` or native. |
...
> **A comparison inside a `triggerOnRandomChance` formula is a dead trigger.** `"($x > 2) * 100"` and `"((($a > 1) + ($b > 1)) > 0) * 100"` never fire. The mechanism is confirmed (2026-08-22, re-export diff + World Debug): the condition survives import unchanged, but a comparison yields **text**, so the arithmetic errors — `Cannot apply '*' to text and a number`, `Cannot apply '+' to text values` — World Debug logs "Its condition couldn't be worked out, so the trigger didn't run", and the trigger sits at "not yet fired" forever. Treat the formula field as a numeric expression. For conditional odds use `if($x > 2, 100, 0)` or `choose(…)`, both of which return numbers; for anything that is really a gate, use `triggerOnPawScript`.
...
2. **Evaluation order matters.** `triggerPrereqs` and `triggerBlockers` only recognize triggers that fired earlier in the same evaluation pass (same turn, earlier in the list). Trigger ordering is therefore semantically significant. Tracked-item state moves within the pass too: a script's writes are visible to everything later in the list — later scripts **and** later triggers' `triggerOnPawScript` conditions (Probe D, 2026-08-22: trigger 13 set a flag and trigger 14's condition on that flag fired the same turn). Triggers run in **list order** (a consumer placed before its producer saw the old state on turn 1 and the new state from turn 2). Only the *storyteller* is deferred to the next turn. Order producers before consumers. Note that a one-shot whose script errored and rolled back still counts as fired for a downstream `triggerPrereqs` — the chain advances even though the state change never landed.
...
4. **Pre-game triggers (`triggerOnStartOfGame: true`).** Fire before turn 0, before the player acts, with or without `triggerConditions` (the flag is its own gate — Probe D, 2026-08-22). Some effects (notably `effectTellAIWhatToDo`) behave differently or are no-ops in this context — the "next turn" they target is turn 1, not turn 0.

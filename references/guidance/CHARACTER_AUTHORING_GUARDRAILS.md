...
- **`img_appearance` and `img_clothing` are author input only.** These drive the visual identity of the character via image generation. Wrong text here produces wildly wrong portraits. The agent must not invent these — prompt the author for the actual descriptors they want, even if it means stopping the workflow to ask.

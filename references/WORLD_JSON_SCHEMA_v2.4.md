...
> **The platform does not migrate the bare array — it deletes it** (confirmed by round trip, 2026-08-06). The condition is destroyed and the trigger keeps its ID, name and effects, leaving `"triggerConditions": []` — which never fires. So **migrate a legacy gate before importing**: the failure mode is a silently dead trigger, with no error in-game and none in the export.

**Both questions this section once carried are now closed by probe runs:** `firedThisTurn` narrows a gate to the current turn (Probe C, 2026-08-29 — see [`triggerConditions[*]`](#triggerconditions)), and `conditions` is an author-maintained registry, not a platform-derived index (Probe A, 2026-08-06 — see §1). Full write-ups in [`probes/README.md`](../probes/README.md).
...
| `recommendedAIModel` | string \| null | Editable | E.g., the platform may have profiles like `"smilodon"`; `null` when no recommendation. No enforced enum at import — IW stores any string verbatim, typos included (Probe E, 2026-08-28); runtime honoring untested. Author real model strings. |
...
| `visibility` | string | Editable | One of: `"everyone"`, `"ai_only"`, `"ai_only_boring"`, `"player_only"`, `"hidden"`, `"hidden_boring"`. `"ai_only"` and `"ai_only_boring"` are equivalent and both may appear in real exports — accept either; preserve whichever the input used on round-trip. `"hidden"` = hidden from both player and AI. `"hidden_boring"` = AI cannot read the item (same readability as `"hidden"`); developer-confirmed, and import survival **confirmed 2026-08-06** by round trip — the value came back unchanged, clearing the [PENDING TEST] the KB carried as of May 2026 (Source: iw_knowledge_base_v2_8.md). |
| `description` | string | Editable | **Always include it** (empty string is fine) — without the key the IW editor silently refuses to open the world (bisected 2026-08-22); `validate_world` errors. Free-form description of what the item represents. May contain `<<template_variables>>` |
...
| `initialValueBasedOnPC` | string | Editable | One of: `"same"` (all characters share initial value), `"character"` (per-character defaults), `"player"` (player chooses at game start). On the tracked item itself all three are valid. On a `possibleCharacters[*].initialTrackedItemValues` **entry**, `"player"` is fatal (IW deletes the entry on import), and so is a `"player"`-scoped backing *item* (entry deleted one round trip later) — per-character entries belong on `"character"`-scoped items only (Probe E, 2026-08-28). Mechanism in [`fields/TRACKED_ITEMS.md`](fields/TRACKED_ITEMS.md). |
...
| `triggerOnTrackedItem` | `{inequality, requiredValue, trackedItemID, textComparison}` | `inequality` one of: `at_least`, `at_most`, `is_exactly`, `not_equal`, `contains`. `not_equal` maps to the UI label "is not exactly"; its import survival is **confirmed 2026-08-06** (round-tripped byte-identical), clearing the [PENDING TEST] the KB carried as of May 2026 — Source: iw_knowledge_base_v2_8.md. `contains` is for text-type comparisons. **`textComparison` is effectively mandatory**: a condition whose `textComparison` is absent or an empty string is **deleted wholesale on import**, whatever the inequality — fixture-proven `at_least` died for lacking the key in the same probe that cleared `not_equal`. `validate_world` errors on absent, empty, and every non-string spelling of "unset" (`null`, `0`, `false`, `[]`). Evidence scope: the probe used one `dataType: "number"` target item, and the canonical fixtures corroborate only weakly (all three carry the same single `triggerOnTrackedItem`, against a `skill_` ref with no `dataType`). Text/yaml-typed targets are untested — set it anyway; the cost of a needless `textComparison` is nil and the cost of a missing one is the whole condition. `requiredValue` may be a formula string like `"1d4+skill_charm"` and must always be a JSON string (not a number — IW crashes on import if non-string). `trackedItemID` may reference a tracked-item ID OR a synthetic `skill_<name>` ID for skill comparisons. `textComparison` is used for text-type tracked items. The condition object also carries top-level `inequality` and `trackedItemID` fields duplicating those inside `data` — preserve both. |
| `triggerOnRandomChance` | `string` (formula) | Formula evaluated each turn, compared against a random number. E.g., a simple `"30"` for 30%, `"15+round(turn_number%random)"`, or — **fixture 1.1 (2026-08)** — `"$number_of_non_human_friends+round(turn_number%random)"`: a tracked item read as `$variableName` so the chance scales with state. Note the mixed dialect: `turn_number` and `random` are bare identifiers here, not `$game.turn_number` / `random(…)`. `validate_world` warns on a non-string/blank formula and on a `$name` that is not a tracked item's `variableName` or a native |
...
| `triggerBlockers` | `{blockers: string[], firedThisTurn: boolean}` | **Shape changed in v2.4** (was a bare `string[]`). `blockers` is the array of trigger IDs that must NOT have fired; if any have, this trigger is blocked. `firedThisTurn` selects the window — see below. |

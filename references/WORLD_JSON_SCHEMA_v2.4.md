...
| `recommendedAIModel` | string \| null | Editable | E.g., the platform may have profiles like `"smilodon"`; `null` when no recommendation. There is **no enforced enum**: IW stores any string verbatim, bogus values included (confirmed 2026-08-28, Probe E). Author real model strings — the platform will preserve a typo without complaint. |
...
| `initialValueBasedOnPC` | string | Editable | One of: `"same"` (all characters share initial value), `"character"` (per-character defaults), `"player"` (player chooses at game start). On the tracked item itself all three are valid. On a `possibleCharacters[*].initialTrackedItemValues` **entry**, `"player"` is fatal — IW deletes the entry on import, keyed on the entry's own scope value whatever the item says (confirmed 2026-08-28, Probe E). Entry-level scope is a projection of the item's on export, so an entry under a `"player"`-scoped item is silently deleted one round trip later — per-character entries belong on `"character"`-scoped items only. See [`fields/TRACKED_ITEMS.md`](fields/TRACKED_ITEMS.md). |

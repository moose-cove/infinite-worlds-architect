...
| `initialValueBasedOnPC` | string | Editable | One of: `"same"` (all characters share initial value), `"character"` (per-character defaults), `"player"` (player chooses at game start). On the tracked item itself all three are valid. On a `possibleCharacters[*].initialTrackedItemValues` **entry**, `"player"` is fatal — IW deletes the entry on import (confirmed 2026-08-06); use `"character"` there, and on its backing item too. The probe held entry-scope and item-scope equal in every cell, so "the entry is what matters" is the working reading, not a separated result — see [`fields/TRACKED_ITEMS.md`](fields/TRACKED_ITEMS.md). |

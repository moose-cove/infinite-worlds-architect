...
|---|---|---|
| `iwdrive.py` | Library + CLI: `worlds`, `export-json --world T out.json`, `import-json --world T in.json`, `snap`. Navigates Menu → world list → Edit → "Show optional features" → "Misc advanced features" → "Show raw JSON", and uses the **Refresh raw JSON** (export) / **Import JSON to world** (import) buttons. | No |
